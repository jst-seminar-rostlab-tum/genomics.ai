...
class Mailer {
  ...
  async send(to: string, subject: string, template_name: string, data: any) {
    ...
    let texttemplate = await fs.readFile(
      path.join(__dirname, "./../../src/views/mails", template_name, "text.txt"),
      "utf-8"
    );
    let htmltemplate = await fs.readFile(
      path.join(__dirname, "./../../src/views/mails", template_name, "html.html"),
      "utf-8"
    );
  ...
  async send_verification_mail(firstname: string, recipient: string, token: string) {
    ...
    return this.send(recipient, "Verify Email", "signup_confirm_email", {
