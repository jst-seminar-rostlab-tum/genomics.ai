...
async function reset_picture_logic(req: ExtRequest, res: Response, updateDatabase: (instId: string) => Promise<string | null | undefined>): Promise<any> {
    try {
...
export function reset_institution_backgroundpicture_route() {
    let router = express.Router();
    router.delete("/institutions/:id/backgroundpicture", check_auth(), async (req: ExtRequest, res) => {
        await reset_picture_logic(req,res, async instId => {
